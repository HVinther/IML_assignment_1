...
Dummy_lrn_SocCat <- po_SocCat_int %>>% po("encode") %>>% po_RecBeg_num %>>% po_RecEnd_rc %>>% po("scale") %>>% lrn_obj |> as_learner()
Target_lrn_SocCat <- po_SocCat_int %>>% po("encodeimpact") %>>% po_RecBeg_num %>>% po_RecEnd_rc %>>% po("scale") %>>% lrn_obj |> as_learner()
```
...
The random forest implementation has some well-performing standard hyper-parameters and we therefore chose to just use these and not perform any tuning of the hyperparameters.
