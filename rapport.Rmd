...
In this section we will investigate the glmnet and gam learners. We will also consider an ensemble learner, where the output from other learners are combined into one.
...
<dt>adhoc:</dt>
<dd>The remaing adhoc encoding described in ..</dd>
<dt>dummy:</dt>
...
For the encodings, using the numeric encoding of the dates as opposed to the default datefeatures also seems preferable. The effect of the remaining encodings are hard to distinguish in this plot. 

For the learners the main thing to note seems to be that tuning build into the cv_glmnet learner faires worse for this dataset compared to the auto tuning of the s parameter, even though this practice is discouraged in the help page for the learner. 
